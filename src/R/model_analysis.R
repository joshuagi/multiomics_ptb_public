...
predictions <- predictions %>%
  bind_rows() %>%
  mutate(
    pair = paste(pmin(inner_id, outer_id), pmax(inner_id, outer_id), sep = "_")
  ) %>%
  mutate(
    model = case_when(
      model %in% c("glmnet_classification", "glmnet_regression") ~ "Lasso",
...
coefs <- coefs %>%
  ...
  mutate(
    pair = paste(
      pmin(inner_id, outer_id, na.rm = TRUE),
...
if (cfg$training$mode == "classification") {
  ...
  calculateRoc <- predictions %>%
    ...
    purrr::map(., function(x) {
      ...
      r.stats <- data.frame(r$ci) %>%
        t() %>%
        data.frame() %>%
        dplyr::rename(ci.lower = X1, auc = X2, ci.upper = X3) %>%
        mutate(
          label = paste0(
            "AUC = ",
            round(auc, 2),
      ...
      r.stats <- r.stats %>%
        mutate(
          label = paste0(
            ...
            round(r.stats$auc, 2),
            " (",
            round(r.stats$ci.lower, 2),
            "-",
            round(r.stats$ci.upper, 2),
            ")"
          )
        ) %>%
        mutate(stimulation = stimulation_i, model = model_i, U = U_i)

      r <- smooth(r, method = "density", bw = 0.001)
      r.data <- data.frame(x = 1 - r$specificities, y = r$sensitivities) %>%
        mutate(stimulation = stimulation_i, model = model_i, U = U_i) %>%
        dplyr::slice(n():1)

      result <- list(r.stats = r.stats, r.data = r.data)
      return(result)
    })
  ...
  r.stats <- r.stats %>%
    ...
    mutate(x = 0.3, y = seq(0, 0.56, by = 0.04)) %>%
      ...
      r.stats <- x %>%
        dplyr::summarise(
          ...
          p_value = suppressWarnings(
            cor.test(response, pred, method = "spearman", exact = FALSE)$p.value
          )
        ) %>%
        mutate(stimulation = stimulation_i, model = model_i, U = U_i)
  ...
  p.value <- r.stats %>%
    ...
    mutate(
      metric = factor(metric, levels = c("n", "spearman", "p_value", "rmse"))
    ) %>%
    arrange(desc(metric)) %>%
    mutate(label = paste0(metric, " = ", round(value, 3))) %>%
    mutate(x = 38, y = seq(0, 0.03, by = 0.01) * 40 + 32)
...
SG_coefs <- coefs %>%
  ...
  mutate(selected = case_when(estimate != 0 ~ 1, TRUE ~ 0)) %>%
  ...
  mutate(
    stimulation = str_split(term, pattern = "_", simplify = T)[, 1],
    model = str_split(term, pattern = "_", simplify = T)[, 2]
  ) %>%
  mutate(
    model = case_when(
      model %in% c("glmnet") ~ "Lasso",
...
total_hits <- coefs %>%
  ...
  purrr::map(., function(x) {
    x <- x %>%
      ...
      pivot_longer(
        cols = -c(term, model, stimulation, U),
        names_to = "pair",
        values_to = "estimate"
      ) %>%
      # ---- Calculate median coefficient / importance scores and the frequency of selection ----
      mutate(
        estimate = case_when(is.na(estimate) == T ~ 0, TRUE ~ estimate)
      ) %>%
      mutate(selected = case_when(estimate != 0 ~ 1, TRUE ~ 0)) %>%
